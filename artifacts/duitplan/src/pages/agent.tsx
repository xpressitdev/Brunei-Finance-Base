    ...
    <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:bg-amber-950/20 dark:border-amber-800">
      ...
      <div className="grid grid-cols-2 gap-4 mb-3">
        <div className="rounded-lg bg-white dark:bg-background p-3 border border-amber-100 dark:border-amber-900">
          ...
          <div className="text-xl font-bold text-foreground">{fmt(parseFloat(data.seen))}</div>
        ...
        <div className="rounded-lg bg-white dark:bg-background p-3 border border-amber-100 dark:border-amber-900">
          ...
          <div className="text-xl font-bold text-foreground">{fmt(parseFloat(data.recorded))}</div>
      ...
      <div className={cn(
        ...
        <span className="text-sm font-medium">
          {isNegative ? "Missing" : "Extra"}: <span className={cn("font-bold", isNegative ? "text-red-700 dark:text-red-400" : "text-green-700 dark:text-green-400")}>{fmt(Math.abs(delta))}</span>
        </span>
      ...
      <div className="space-y-1.5">
        {data.slice(0, 4).map((item, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <div className="text-xs text-white/70 w-14 text-right">{fmt(item.value)}</div>
          </div>
        ))}
      </div>
    </div>
